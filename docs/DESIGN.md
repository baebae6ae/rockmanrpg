...
| `tools/detect_frames.py` | 리핑 시트에서 프레임 자동 검출·정규화 | 컨테이너 |
| `tools/gen_placeholder.py` | 규격에 맞는 임시 도트 생성 | 컨테이너 |
| `tools/validate_data.ts` | 데이터 스키마 검증 | 컨테이너 |
...
│   └── maps/                  # 맵 메타데이터, 스폰 테이블
├── assets/
│   ├── sprites/               # 정규화된 진짜 스프라이트 (버전 관리 X)
│   ├── raw/                   # 원본 리핑 시트, 임포트 전 (버전 관리 X)
│   └── generated/             # 스크립트로 만든 임시 도트 (버전 관리 X, 재생성 가능)
└── tools/                     # 프레임 검출, 임시 도트 생성, 데이터 검증
```

`assets/generated/`는 `tools/gen_placeholder.py`로 언제든 재생성되므로 버전 관리하지 않는다.
