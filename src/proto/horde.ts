...
import {
  ...
  BEATS, ELEM_COLOR, ELEM_NAME, ELEM_WORD, WEAK_MULT, elemMult,
  GROW_CAP, pickKind, rollWaveElem,
...
export async function runHordeProto(app: Application, input: Input): Promise<void> {
  ...
  if (DEBUG) {
    ...
    // dbg.__hordePickIndex 는 매 프레임 실제 값으로 덮이는 읽기 전용
    // 내보내기다 — 외부에서 window.__hordePickIndex 를 직접 대입해도
    // 다음 프레임에 바로 원래 값으로 돌아가 아무 효과가 없다. 헤드리스
    // 테스트에서 특정 카드를 실제로 고르려면 이 훅이 필요하다.
  ...
  function draw(dt: number): void {
    ...
    if (lungeMoveT > 0 && phase === 'play') {
      ...
      // 칼날이 몸 주위를 도는 모양으로 만든다. arc()/circle() 은
      // 매끈하게 갈려서 다른 도트 이펙트와 결이 안 맞았다 — pxArcBand/
      // pxDisc/pxLine 으로 바꾼다.
      const t = clamp(1 - lungeMoveT / LUNGE_MOVE_DUR, 0, 1);
    ...
    if (chargeBeamT > 0 && phase === 'play') {
      ...
      for (let bi = 0; bi < chargeBeamAngles.length; bi++) {
        ...
        if (chargeLook === 'drive') {
          ...
          for (let s = 0; s < 3; s++) {
            ...
            pxRing(specialG, px + c * visible * t, hy + sn * visible * t, 5, 1, 1, 2);
          }
          specialG.fill({ color: 0x0a0a12, alpha: 0.7 * k });
          specialG.beginPath();
          for (let s = 0; s < 3; s++) {
            const t = 1 - ((animClock * 2.2 + s / 3) % 1);
      ...
      specialG.fill({ color: 0xffffff, alpha: 0.85 });
    }
